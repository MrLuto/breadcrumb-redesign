...
import { useCart, getCartItemPrice, formatSelectedOptions } from '@/hooks/useCart';
...
export function CartSheet() {
  ...
  const toggleNotes = (cartItemKey: string) => {
    setOpenNotes(prev => ({ ...prev, [cartItemKey]: !prev[cartItemKey] }));
  };
            ...
            <ScrollArea className="flex-1 -mx-6 px-6">
              <div className="space-y-4 py-4">
                {items.map((item) => {
                  ...
                  return (
                    <div key={item.cartItemKey} className="flex gap-4">
                      {/* Product Image */}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Input
                            ...
                            value={item.quantity}
                            onChange={(e) =>
                              updateQuantity(item.cartItemKey, parseInt(e.target.value) || 1)
                            }
                        ...
                        {/* Notes per item */}
                        <Collapsible open={openNotes[item.cartItemKey] || !!item.notes}>
                          <CollapsibleTrigger asChild>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="mt-2 h-7 px-2 text-xs text-muted-foreground hover:text-foreground"
                              onClick={() => toggleNotes(item.cartItemKey)}
                            >
                              <MessageSquare className="h-3 w-3 mr-1" />
                              {item.notes ? 'Opmerking bewerken' : 'Opmerking toevoegen'}
                              <ChevronDown className={`h-3 w-3 ml-1 transition-transform ${openNotes[item.cartItemKey] ? 'rotate-180' : ''}`} />
                            </Button>
                          </CollapsibleTrigger>
                          <CollapsibleContent>
                            <Textarea
                              ...
                              value={item.notes || ''}
                              onChange={(e) => updateNotes(item.cartItemKey, e.target.value)}
